...
In order to do this, we present a function (``listMovies``) that the user can call with a search string and a limit number (parameter we want to add) from the list of movies that will be returned corresponding to that string, since the string can be incomplete and therefore potentially matching to more than one movie in the site's database. The list returned by the function is ordered decreasingly by the order of confidence, where we stipulate to be the rating of the films.

``listMovies`` is aided by a set of other functions presented (``Searchform``, ``listMoviesAll`` e ``webPages``), which, respectively, help to: submit the form in `imdb.com` with the search string, find the list of all the movies that match the string that the site presents, and get the links from the web pages of all those movies.
...
The goal now is to build a model that can predict the grade of the review based on your text.

####Build a data set for learning

For this, we will first construct a data set (``dataSet``) suitable for learning of this model(s).

First, let's build a corpus of directories with the reviews of the movie (we will continue with the previous example of the film "Lego") so that we can perform the following preprocessing tasks (Clean-ups).
...
Later we will try the following models: SVMs (linear and radial kernel), Tree-based Models and MARS.
...
```{r, eval=FALSE}
dataSetTest <- dataSet[which(is.na(dataSet$class)), ]       #Save lines with NA's to in the end predict your star
...
Having said this, we will predict (using these two methods) the ratings of previously saved reviews (reviews whose users did not assign stars).

```{r, eval=FALSE}
##SVM radial kernel
modelSVM2 <- svm(class ~ ., dataSet, kernel = "radial")
(predSVM2 <- predict(modelSVM2, dataSetTest))             ##Porque não preve? escrever conclusoes
...
But we should note that this model has an average absolute deviation between the predictions and true values of 3.014062 which means that this 5-star rating may actually be approximately 2 stars or 8 stars.

We can then conclude that even the best error values (MAE of 2.791419 and MSE of 10.85248) obtained here are still quite relevant for this problem itself, given that the scale we have is 10 (stars) and an average absolute deviation of almost 3 units is quite relevant for the consideration of a positive opinion, an average opinion and a negative opinion.
