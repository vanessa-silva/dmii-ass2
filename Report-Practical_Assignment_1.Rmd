...
ratingOfReview <- function(x) {
  ans <- NULL
  cnt <- 1
  for(i in 1:length(x)) {
    if(!is.na(tmp[i] %>% html_attr("class")) && tmp[i] %>% html_attr("class") == "avatar") {
      if(i == length(x)) {
        ans[cnt] <- NA
        cnt <- cnt + 1
      }
      else {
        if(!is.na(tmp[i + 1] %>% html_attr("class")) && tmp[i + 1] %>% html_attr("class") == "avatar") {
          ans[cnt] <- NA
          cnt <- cnt + 1
        }
      }
